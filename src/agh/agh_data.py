...
class GraderOptions(DataclassJson):
    ...
    @classmethod
    def load_user_defaults(cls):
        user_defaults_file = Path.home() / ".config" / "agh" / ".agh_user_defaults.json"
    ...
    def editTemplate(self) -> None:
        ...
        os.system(self.template_editor_command.substitute(file=self.output_template_name, assignment_directory=assignment.root_directory))
    ...
    def editTests(self) -> None:
        ...
        assignment = Assignment.load()
        os.system(self.test_editor_command.substitute(file=self.default_test_file_name, assignment_directory=assignment.root_directory))
...
@dataclass(kw_only=True)
class SubmissionData(DataclassJson):
