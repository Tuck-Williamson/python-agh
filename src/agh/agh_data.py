...
import json
import pathlib
from dataclasses import dataclass, KW_ONLY, field, asdict, is_dataclass, fields
from typing import get_args, get_origin, Self, Any, Iterable
...
class Submission(submission_data):
  ...
  def new(cls, assignment: Assignment, submission_file: pathlib.Path) -> Self:
    ...
    ret_val = cls(submission_file=my_submission_file, evaluation_directory=evaluation_directory,
                  anon_name=anon_name, original_name=submission_file.name)
  ...
  def get_anon_name(cls, assignment: Assignment, submission_file: pathlib.Path):
    ...
    """
    return anonimizer.anonymize(submission_file.name, assignment.name, assignment.year, assignment.grade_period,
                                assignment.course)
