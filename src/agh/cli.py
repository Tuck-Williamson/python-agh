...
from agh import Assignment, Submission
...
def SubFileCompleter(property:str, prefix:str, **kwargs):
    try:
        ...
        ret_val = [str(glob_item) for glob_item in directory.relative_to(cur_dir, walk_up=True).glob(f'*', case_sensitive=False) if glob_item.is_file()]
...
parser = MyArgParser(description="agh --- Assignment Grading Helper", prog="agh", formatter_class=RichHelpFormatter, conflict_handler="resolve")
...
assign_add_required_parser.add_argument("-i", "--include-in-output", help="Include in output", action=argparse.BooleanOptionalAction, default=True)
...
assign_add_required_parser.add_argument("-i", "--include-in-output", help="Include in output", action=argparse.BooleanOptionalAction, default=False)
...
sub_add_subparser.add_argument("files", nargs='+', help="Submission files to add", type=Path).completer = functools.partial(SubFileCompleter, "unprocessed_dir")

# submission > fix command
sub_fix_subparser = sub_subparsers.add_parser("fix", help="Fix a submission. Try this if you accidentally deleted something. This may re-create links etc.")
sub_fix_subparser.add_argument("files", nargs='+', help="Submission files to add", type=str).completer = lambda **kwargs: [subm.evaluation_directory.name for subm in get_current_assignment().Submissions]
...
def get_current_assignment() -> Assignment:
    try:
        ...
        console.print(f'[error]No assignment found.')
        sys.exit(1)
