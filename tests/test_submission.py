...
from agh.agh_data import submission_data, Submission, Assignment
...
class TestSubmissionData(unittest.TestCase):
  ...
  def test_eq(self):
    ...
    s1 = self.new_sub(anon_name="test", original_name='bob.tar.gz', compiled_initially=True,
                      initial_missing_files=['a.c', 'b.c', 'b.h'])
    s1_dup = self.new_sub(anon_name=s1.anon_name, original_name=s1.original_name,
                          compiled_initially=s1.compiled_initially, initial_missing_files=s1.initial_missing_files)
    self.assertEqual(s1, s1_dup)

    s2 = self.new_sub(anon_name="test2", original_name='bob.tar.gz', compiled_initially=True,
                      initial_missing_files=['a.c', 'b.c', 'b.h'])
...
class TestSubmission(unittest.TestCase):
  def setUp(self):
    ...
    self.sub_file.touch()
    self.sub_data = submission_data(submission_file=self.sub_file, evaluation_directory=self.base, anon_name="test", original_name='bob.tar.gz')
...
class TestTarSubmission(TestSubmission):
  ...
  def test_new_save_load(self):
    ...
    for f in self.tar_files:
      self.assertTrue((s1.as_submitted_dir/f.name).exists(), f'File in tar not found in as submitted directory.{'\n\t'.join([str(pth) for pth in s1.as_submitted_dir.iterdir()])}')
